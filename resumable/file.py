...
FileChunk = namedtuple('FileChunk', ['index', 'size', 'read','offset','total'])
...
def build_chunks(read_bytes, file_size, chunk_size):
    ...
    while start < file_size:
        ...
        chunk = FileChunk(index, size, partial(read_bytes, start, size),start,file_size)
        chunks.append(chunk)
...
class ResumableFile(object):
    ...
    def __init__(self, path, chunk_size,unique_identifier):

        self.path = str(path)
        self.unique_identifier = unique_identifier#uuid.uuid4()
