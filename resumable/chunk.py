...
def _send_chunk(session, config, file, chunk):
    ...
    """
    data_toPost=_build_query(file, chunk)
    response = session.post(
        config.target,
        data=data_toPost,
        files={'qqfile': chunk.read()}
    )
...
def _build_query(file, chunk):
    ...
    return {
        'qqchunksize': chunk.size,
        'qqtotalfilesize': file.size,
        'qqtype': _file_type(file.path),
        'qquuid': str(file.unique_identifier),
        'qqfilename': os.path.basename(file.path),
        'qqfilepath': file.path,
        'qqtotalparts': len(file.chunks),
        'qqpartindex': chunk.index ,#+ 1,
        'qqpartbyteoffset': chunk.offset
    }
